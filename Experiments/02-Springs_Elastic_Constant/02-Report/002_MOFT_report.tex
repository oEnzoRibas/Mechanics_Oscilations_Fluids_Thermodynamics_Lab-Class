...
colocar um objeto de massa  na outra extremidade, ocorre um alongamento (b).}
\label{fig:setup1}
...
Foi feita uma média aritmética dos tempos registrados para cada altura, e os valores médios foram utilizados para a análise dos dados.
...
Pelo ajuste não linear dos dados no gráfico $h$ vs $t$, o valor de $g$ pode ser determinado a partir do coeficiente do termo quadrático. Mas essa representação não é a mais adequada para a determinação de $g$ a partir de um ajuste de curva, pois envolve o ajuste não linear, que requer de métodos iterativos e uma boa estimativa inicial, enquanto ajustes lineares podem ser resolvidos de forma mais simples e direta \citep{bevington2003data}.
...
em que \(A = \frac{g}{2}\) é o coeficiente angular da reta e \(b\) é um deslocamento fixo que corrige a posição inicial medida pelo sensor, já que o ponto exato de detecção não pode ser definido com precisão. Em termos de \(t\), a relação original é
...
\toprule
{\textbf{Tempo $t \pm 0,0005$ (s)}} & {\textbf{Altura $h \pm 0,005$ (m)}} \\
\midrule
    0.115 & 0.05 \\
    0.141 & 0.08 \\
    0.168 & 0.12 \\
...
\bottomrule
\end{tabular}
\caption{Dados experimentais de tempo e altura com incertezas instrumentais fixas.}
